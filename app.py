...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename= time.strftime('logs/'+'%Y-%m-%d-%H.%M.%S',time.localtime(time.time()))+'.log',
                    filemode='a')
...
def get_state(payload,max_len):
    payload= payload.strip()
    if len(payload) >= max_len:
        state = np.fromstring(payload, dtype=np.uint8,count=max_len)
    else:
        state = np.fromstring(payload, dtype=np.uint8)
        state = np.append(state,np.zeros(max_len-len(payload)))
    return state
...
class DQN:
    ...
    def __init__(self,max_len,n_actions,batch_size=32,memory_size=1000,episode=10000,target_update=10,EPSILON = 0.1,GAMMA = 0.95 ,learning_rate = 0.01,ddqn=False):
    ...
    def compute_loss(self,states, actions, rewards,dones,next_states): 
        ...
        if self.ddqn: 
            ...
            q_next = tf.gather(self.target_model(next_states),q_next_action,batch_dims=1)
        else:
    ...
    def train_step(self,model, loss_function, optimizer, custom_fwd_fn=None):
        ...
        with tf.GradientTape() as tape:
            loss = loss_function(state_batch, action_batch, reward_batch,done_batch,next_state_batch)   
    ...
    def start_training(self,initial_state,save_path ):
        ...
        self.plotter = mdl.util.PeriodicPlotter(sec=2, xlabel='Iterations', ylabel='Rewards')
        ...
        with open(save_path+'meta.txt','w') as f:
            f.write(str(self.__dict__))     
    
    def train(self,state,action,next_state, reward, done):
        self.total_reward+=reward
        logging.info('train episode:'+str(self.i_episode)+'total_reward:'+str(self.total_reward))
        t = 1 if done else 0
        self.memory.push(state, action, reward,t,next_state)
        self.train_step(self.eval_model, self.compute_loss, self.optimizer)
        if done:
            logging.info('episode：'+str(self.i_episode)+'finished'+'total_reward:'+str(self.total_reward))
            self.smoothed_reward.append(self.total_reward)
            if self.best_reward < self.total_reward:
                self.best_reward = self.total_reward
                self.eval_model.save_weights(self.save_path)
                logging.info('model saved at :' + str(self.save_path)+'with reward:'+str(self.total_reward))
            self.plotter.plot(self.smoothed_reward.get())
            self.total_reward = 0
            self.i_episode+=1
...
class PolicyGradient:
    ...
    def __init__(self,max_len,n_actions,episode=10000,GAMMA = 0.95 ,learning_rate = 0.01):
        self.n_actions = n_actions
        self.max_len = max_len
        self.episode = episode
        self.GAMMA=GAMMA
    ...
    def train(self,state,action,next_state, reward, done):
        ...
        if done:
            ...
            logging.info('train episode:',self.i_episode,'finished.Total reward:',total_reward)
            
            if self.best_reward < total_reward:
                self.best_reward = total_reward
                self.model.save_weights(self.save_path)
                logging.info('model saved at :' , self.save_path,'with reward:',total_reward)
...
class QLearning:
    ...
    def __init__(self,max_len,n_actions,state_size=100000,episode=60000,EPSILON=1,GAMMA = 0.95,EPSILON_decay=0.995,learning_rate = 0.01):
        self.max_len = max_len
        ...
        self.learning_rate = learning_rate
        self.q_table =  np.random.uniform(low=0, high=1, size=(state_size + [n_actions]))
    ...
    def train(self,state,action,next_state, reward, done):
        ...
        if not done:
            ...
            self.plotter.plot(self.smoothed_reward.get())
            logging.info('train episode:',self.i_episode,'finished.Total reward:',self.total_reward)
            
            if self.best_reward < self.total_reward:
                self.best_reward = self.total_reward
                np.save(self.save_path, self.q_table)
                logging.info('model saved at :' , self.save_path,'with reward:',self.total_reward)
                
            if self.EPSILON > 0.05 and self.episode > 10000:
                    self.EPSILON = math.pow(self.EPSILON_decay_, self.episode - 10000)
...
#     state_size = int(data.get('state_size',100000))  
...
#     model = QLearning(max_len,n_actions,state_size,episode,EPSILON,GAMMA,EPSILON_decay,learning_rate)
...
def init_dqn():
    ...
    model = DQN(max_len,n_actions,batch_size,memory_size,episode,target_update,EPSILON ,GAMMA,learning_rate,ddqn)
...
def initPolicyGradient():
    ...
    GAMMA = float(data.get('GAMMA',0.95)) 
    learning_rate =float (data.get('learning_rate',0.01))
    model = PolicyGradient(max_len,n_actions,episode,GAMMA,learning_rate)
...
def start_training():                 
    ...
    if state is None:
        return jsonify({'code': 400,'msg':'Error! Initial state not provided!! Please add "initial_state" argument'})
    default_path = './checkpoints/'+time.strftime('%Y-%m-%d-%H.%M.%S',time.localtime(time.time()))+'/'
    save_path = data.get('save_path',default_path) 
    model.start_training(get_state(state,model.max_len),save_path)
...
def train_step():      
    ...
    if state is None:
        return jsonify({'code': 400,'msg':'Error! state not provided!! Please add "state" argument.'})
    state = get_state(state,model.max_len)
    action = data.get('action')
    if action is None:
        return jsonify({'code': 400,'msg':'Error! state not provided!! Please add "action" argument.'})
    next_state = data.get('next_state')
    if  next_state is None:
        return jsonify({'code': 400,'msg':'Error! state not provided!! Please add "next_state" argument.'})
    next_state = get_state(next_state,model.max_len)
    reward = data.get('reward')
    if reward is None:
        return jsonify({'code': 400,'msg':'Error! state not provided!! Please add "reward" argument.'})
    done = data.get('done')
    if done is None:
        return jsonify({'code': 400,'msg':'Error! state not provided!! Please add "done" argument.'})
...
def setParameter():
    ...
    if not model:
        ...
        data = request.get_json()
        logging.info("/setParamete with data")
        logging.info(data)
        if data is None:
